...
async function applyEdit(filePath: string, content: string, baseVersion: number, source?: WebSocket, requestId = crypto.randomUUID(), sourceId = clientId) {
  const target = safePath(filePath); if (!target) throw new Error('Caminho de arquivo inválido.'); if (content.length > 1024 * 1024) throw new Error('Arquivo colaborativo excede 1 MB.');
  ...
  const payload = { path: filePath, content, version, clientId: sourceId, requestId }; broadcast({ type: 'edit', payload }, source); emit({ type: 'file', payload }); return true;
}
...
export async function sendCollabEdit(filePath:string,content:string,baseVersion=versions.get(filePath)??0){if(mode==='host'){await applyEdit(filePath,content,baseVersion);return;}if(socket?.readyState===WebSocket.OPEN){socket.send(JSON.stringify({type:'edit',path:filePath,content,baseVersion,requestId:crypto.randomUUID()}));return;}throw new Error('Nenhuma sessão conectada.');}
