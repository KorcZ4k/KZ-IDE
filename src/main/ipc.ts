...
export function registerIpc(){
 ...
 ipcMain.handle('window:toggle-maximize',event=>{const w=BrowserWindow.fromWebContents(event.sender);if(w?.isMaximized())w.unmaximize();else w?.maximize();});
 ipcMain.handle('window:close',event=>BrowserWindow.fromWebContents(event.sender)?.close());
 ipcMain.handle('window:icon',event=>{const w=BrowserWindow.fromWebContents(event.sender);return w?.getTitle()??'Aurora'});
