...
const attachModel = (model: monaco.editor.ITextModel) => {
  const path = relativePath(modelPath(model));
  if (!path || subscriptions.has(model.uri.toString())) return;
  bases.set(path, model.getValue());
  const subscription = model.onDidChangeContent(() => {
    if (suppress) return;
    const content = model.getValue(); const base=versions.get(path)??0;
...
const threeWayMerge=(base:string,local:string,remote:string)=>{
  if(local===base)return remote;if(remote===base)return local;if(local===remote)return local;
  const b=base.split('\n'),l=local.split('\n'),r=remote.split('\n'),max=Math.max(b.length,l.length,r.length),out:string[]=[];
  for(let i=0;i<max;i++){const bv=b[i]??'',lv=l[i]??'',rv=r[i]??'';if(lv===rv)out.push(lv);else if(lv===bv)out.push(rv);else if(rv===bv)out.push(lv);else out.push('<<<<<<< LOCAL',lv,'=======',rv,'>>>>>>> REMOTE');}
  return out.join('\n');
};

const applyRemote=(payload:FilePayload)=>{
  if(!payload?.path||typeof payload.content!=='string')return;versions.set(payload.path,Number(payload.version)||0);
  const model=getModel(payload.path);if(!model){bases.set(payload.path,payload.content);return;}
...
const applyConflict=(payload:ConflictPayload)=>{
  if(!payload?.path||typeof payload.content!=='string')return;const model=getModel(payload.path),pendingEdit=pending.get(payload.path);versions.set(payload.path,Number(payload.version)||0);
  if(!model||!pendingEdit){bases.set(payload.path,payload.content);notice(`Conflito em ${payload.path}. Workspace atualizado.`);return;}
  const merged=threeWayMerge(pendingEdit.base,pendingEdit.local,payload.content);suppress++;try{model.setValue(merged);bases.set(payload.path,payload.content);pending.delete(payload.path);}finally{suppress--;}
  if(merged.includes('<<<<<<< LOCAL'))notice(`Conflito em ${payload.path}: marcadores de merge foram inseridos.`);else{void window.kz.collab.edit(payload.path,merged,Number(payload.version)||0).catch(()=>undefined);notice(`Conflito em ${payload.path} reconciliado automaticamente.`);}
};
