...
const getModel = (path: string) => monaco.editor.getModels().find(model => modelPath(model) === path) ?? null;
...
const attachModel = (model: monaco.editor.ITextModel) => {
  ...
  model.onWillDispose(() => { subscription.dispose(); subscriptions.delete(model.uri.toString()); bases.delete(path); pending.delete(path); });
};
...
const threeWayMerge = (base: string, local: string, remote: string) => {
  ...
  for (let i = 0; i < max; i++) { const bv=b[i]??''; const lv=l[i]??''; const rv=r[i]??''; if(lv===rv)out.push(lv); else if(lv===bv)out.push(rv); else if(rv===bv)out.push(lv); else out.push('<<<<<<< LOCAL',lv,'=======',rv,'>>>>>>> REMOTE'); }
  return out.join('\n');
};
...
const applyRemote = (payload: FilePayload) => {
  ...
  if (model.getValue() === payload.content) { bases.set(payload.path, payload.content); return; }
  suppress++;
  try { model.setValue(payload.content); bases.set(payload.path, payload.content); pending.delete(payload.path); } finally { suppress--; }
};

const applyConflict = (payload: ConflictPayload) => {
  if (!payload?.path || typeof payload.content !== 'string') return;
  const model = getModel(payload.path); const pendingEdit = pending.get(payload.path); versions.set(payload.path, Number(payload.version) || 0);
  if (!model || !pendingEdit) { bases.set(payload.path, payload.content); notice(`Conflito em ${payload.path}. Workspace atualizado.`); return; }
  const merged = threeWayMerge(pendingEdit.base, pendingEdit.local, payload.content); suppress++;
  try { model.setValue(merged); bases.set(payload.path, payload.content); pending.delete(payload.path); } finally { suppress--; }
  if (merged.includes('<<<<<<< LOCAL')) notice(`Conflito em ${payload.path}: marcadores de merge foram inseridos.`);
  else { void window.kz.collab.edit(payload.path, merged, Number(payload.version) || 0).catch(() => undefined); notice(`Conflito em ${payload.path} reconciliado automaticamente.`); }
};

const applyAck = (payload: FilePayload) => {
  if (!payload?.path) return;
  versions.set(payload.path, Number(payload.version) || 0);
  const model = getModel(payload.path); if (model) bases.set(payload.path, model.getValue()); pending.delete(payload.path);
};
...
const wireEditor = (editor: monaco.editor.ICodeEditor) => {
  const sendCursor = () => { const model=editor.getModel(); const position=editor.getPosition(); if(!model||!position)return; void window.kz.collab.cursor({ path:modelPath(model), position, selection:editor.getSelection() }); };
  editor.onDidChangeCursorPosition(sendCursor); editor.onDidChangeCursorSelection(sendCursor); sendCursor();
};

const scan = () => {
  monaco.editor.getModels().forEach(attachModel);
  monaco.editor.getEditors().forEach(editor => { const dom=editor.getDomNode(); if(dom && dom.dataset.auroraCollabWired!=='true'){dom.dataset.auroraCollabWired='true';wireEditor(editor);} });
};

window.kz.collab.onEvent(event => {
  if(event.type==='file'){const payload=event.payload as FilePayload;if(payload?.path)applyRemote(payload);}
