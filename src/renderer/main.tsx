...
function App() {
  ...
  const refresh = async (workspace = root) => {
    if (!workspace) return;
    try {
      setTree(await window.kz.workspace.readTree(workspace));
      setGitStatus(await window.kz.git.status(workspace));
  ...
  const openFile = async (node: FileNode, line?: number) => {
    ...
    try {
      ...
      setOpenFiles(f => [...f, { path: node.path, name: node.name, content, dirty: false }]);
      setActivePath(node.path); setNotice(line ? `Linha ${line}` : node.name);
  ...
  const createFile = async () => {
    ...
    if (!name?.trim()) return;
    await window.kz.file.create(`${root}/${name.trim()}`, 'file'); await refresh(); setNotice(`Criado: ${name.trim()}`);
  };

  const removeActive = async () => {
    if (!active) return;
    if (settings.confirmDelete && !window.confirm(`Excluir ${active.name}?`)) return;
    await window.kz.file.remove(active.path); setOpenFiles(fs => fs.filter(f => f.path !== active.path)); setActivePath(null); await refresh(); setNotice('Arquivo removido');
  };
  ...
  useEffect(() => {
    void Promise.all([window.kz.workspace.last(), window.kz.settings.get()]).then(([state, saved]) => {
      setRecent(state.recent); setSettings(saved); if (state.workspace) { setRoot(state.workspace); void refresh(state.workspace); }
    });
  }, []);
  ...
  return <div className="app">
    <header className="titlebar"><div className="brand"><span className="brand-mark">KZ</span><strong>KZ-IDE</strong></div><nav className="menu"><button onClick={() => void openWorkspace()}>File</button><button onClick={() => setPalette(true)}>Edit</button><button onClick={() => setFileSearch(true)}>View</button><button onClick={() => setTerminalOpen(o => !o)}>Run</button><button onClick={() => setTerminalOpen(o => !o)}>Terminal</button><button onClick={() => setSettingsOpen(true)}>Preferences</button></nav><div className="workspace-name">{root ? root.split(/[\\/]/).pop() : 'sem workspace'}</div></header>
    <main className="workspace"><aside className="activitybar"><button className="activity active" title="Explorer">▤</button><button className="activity" title="Pesquisar no projeto" onClick={() => setContentSearch(true)}>⌕</button><button className={`activity ${gitPanel ? 'active' : ''}`} title="Source Control" onClick={() => setGitPanel(p => !p)}>⑂{gitStatus && !gitStatus.clean && <i>{gitStatus.files.length}</i>}</button><button className={`activity ${problemsOpen ? 'active' : ''}`} title="Problemas" onClick={() => setProblemsOpen(p => !p)}>⚠</button><button className="activity" title="Run and Debug" onClick={() => setTerminalOpen(true)}>▷</button><button className="activity" title="Configurações" onClick={() => setSettingsOpen(true)}>⚙</button></aside>
      <aside className="sidebar">{gitPanel ? <GitPanel status={gitStatus} onAction={gitAction} /> : <><div className="sidebar-head"><span>EXPLORER</span><div><button title="Abrir pasta" onClick={() => void openWorkspace()}>⌂</button><button title="Novo arquivo" onClick={() => void createFile()}>＋</button></div></div><div className="section-title">{root ? root.split(/[\\/]/).pop() : 'ABRA UM WORKSPACE'}</div>{root ? <FileTree nodes={tree} onOpen={openFile} /> : <div className="empty"><p>Um IDE leve para trabalhar com pastas reais, Git e ferramentas de desenvolvimento.</p>{recent.length > 0 && <div className="recent"><small>RECENTES</small>{recent.slice(0, 5).map(p => <button key={p} onClick={() => void openRecent(p)}>{p.split(/[\\/]/).pop()}<span>{p}</span></button>)}</div>}<button className="primary" onClick={() => void openWorkspace()}>Abrir pasta</button></div>}</>}</aside>
      <section className="main-area"><div className="tabs">{openFiles.map(file => <button key={file.path} className={`tab ${file.path === activePath ? 'active' : ''}`} onClick={() => setActivePath(file.path)}><span className="file-dot">{file.dirty ? '●' : '•'}</span>{file.name}<span className="close" onClick={e => { e.stopPropagation(); setOpenFiles(fs => fs.filter(f => f.path !== file.path)); if (file.path === activePath) setActivePath(null); }}>×</span></button>)}<button className="new-tab" onClick={() => setPalette(true)}>＋</button></div><div className="editor">{active ? <MonacoEditor file={active} settings={settings} onChange={content => setOpenFiles(fs => fs.map(f => f.path === active.path ? { ...f, content, dirty: true } : f))} onNotice={setNotice} /> : <Welcome onOpen={openWorkspace} onCreate={createFile} />}</div>{terminalOpen && <section className="terminal"><div className="panel-tabs"><span className="panel-active">TERMINAL</span><span className={problemsOpen ? '' : 'clickable'} onClick={() => setProblemsOpen(true)}>PROBLEMS</span><span>OUTPUT</span><button onClick={() => setTerminalOpen(false)}>×</button></div>{problemsOpen ? <Problems openFiles={openFiles} /> : <div className="terminal-output">{terminalLines.map((line, i) => <div key={`${i}-${line}`}>{line}</div>)}<div className="terminal-input"><span>$</span><input value={command} onChange={e => setCommand(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') void runCommand(command); }} placeholder="comando..." /></div></div>}</section>}</section></main>
    ...
    {palette && <CommandPalette onClose={() => setPalette(false)} actions={[["Abrir pasta", openWorkspace], ["Novo arquivo", createFile], ["Salvar arquivo", saveActive], ["Pesquisar no projeto", () => setContentSearch(true)], ["Alternar terminal", () => setTerminalOpen(o => !o)], ["Problemas", () => setProblemsOpen(true)], ["Configurações", () => setSettingsOpen(true)], ["Excluir arquivo ativo", removeActive], ["Source Control", () => setGitPanel(true)]]} />}
...
function MonacoEditor({ file, settings, onChange, onNotice }: { file: OpenFile; settings: KZSettings; onChange: (content: string) => void; onNotice: (s: string) => void }) {
  ...
  useEffect(() => {
    ...
    const editor = monaco.editor.create(ref.current, { model, theme: 'kz-dark', automaticLayout: true, minimap: { enabled: settings.minimap }, fontSize: settings.fontSize, wordWrap: settings.wordWrap, tabSize: 2, padding: { top: 14 }, smoothScrolling: true, scrollBeyondLastLine: false });
    const validate = () => {
      ...
      monaco.editor.setModelMarkers(model, 'kzide', markers);
      onNotice(markers.length ? `${markers.length} problema(s)` : 'Sem problemas');
    };
    const d = editor.onDidChangeModelContent(() => { change.current(editor.getValue()); validate(); });
    validate();
...
function SettingsDialog({ settings, onChange, onClose }: { settings: KZSettings; onChange: (p: Partial<KZSettings>) => void; onClose: () => void }) { return <div className="overlay"><div className="settings"><div className="settings-head"><strong>KZ-IDE Settings</strong><button onClick={onClose}>×</button></div><label>Fonte <input type="number" min="10" max="24" value={settings.fontSize} onChange={e => void onChange({ fontSize: Number(e.target.value) })} /></label><label>Minimap <input type="checkbox" checked={settings.minimap} onChange={e => void onChange({ minimap: e.target.checked })} /></label><label>Quebra de linha <select value={settings.wordWrap} onChange={e => void onChange({ wordWrap: e.target.value as 'off' | 'on' })}><option value="off">Desligada</option><option value="on">Ligada</option></select></label><label>Auto save <input type="checkbox" checked={settings.autoSave} onChange={e => void onChange({ autoSave: e.target.checked })} /></label><label>Confirmar exclusão <input type="checkbox" checked={settings.confirmDelete} onChange={e => void onChange({ confirmDelete: e.target.checked })} /></label><div className="settings-note">Preferências persistem no perfil do KZ-IDE. Atalho: Ctrl/Cmd + ,</div></div></div>; }
